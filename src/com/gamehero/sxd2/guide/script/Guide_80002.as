package com.gamehero.sxd2.guide.script
{
	 ...
	 * @create 2014-12-31
	 **/
	public class Guide_80002 extends Guide
	{
		private var wingWindow:GrowupWindow;
		private var wingLevelUpWindow:GrowupLevelUpWindow;
		
		
		/**
		 * 构造函数
		 * */
		public function Guide_80002()
		{
			super();
		...
		override public function playGuide(info:GuideVO, callBack:Function=null, param:Object = null):void
		{
			...
			// 打开翅膀面板
			MainUI.inst.openWindow(WindowEvent.GROWUP_WINDOW , GS_GrowupType_Pro.GROWUP_WING);
			wingWindow = WindowManager.inst.getWindowInstance(GrowupWindow,WindowPostion.CENTER) as GrowupWindow;
			if(wingWindow)
			{
				if(wingWindow.loaded)
				{
					this.next1();
				}
				else
				{
					wingWindow.addEventListener(Event.COMPLETE , next1);
				}
			}
		}
		...
		private function next1(e:Event = null):void
		{
			...
			// 坐骑进阶按钮
			var levelUp_Bt:DisplayObject = wingWindow.leveUp_Bt;
		...
		{
			// 进阶面板
			wingLevelUpWindow = WindowManager.inst.getWindowInstance(GrowupLevelUpWindow,WindowPostion.CENTER) as GrowupLevelUpWindow;
			if(wingLevelUpWindow)
			{
				if(wingLevelUpWindow.loaded)
				{
					this.next3();
				}
				else
				{
					wingLevelUpWindow.addEventListener(Event.COMPLETE , next3);
				}
			}
		}
		...
		private function next3(e:Event = null):void
		{
			...
			// 手动进阶按钮
			var btn:DisplayObject = wingLevelUpWindow.autoLeveup_Bt;
		...
		override protected function guideCompleteHandler(e:GuideEvent=null):void
		{
			if(wingWindow)
			{
				wingWindow.removeEventListener(Event.COMPLETE , next1);
				wingWindow = null;
				
			}
			if(wingLevelUpWindow)
			{
				wingLevelUpWindow.removeEventListener(Event.COMPLETE , next3);
				wingLevelUpWindow = null;
			}
